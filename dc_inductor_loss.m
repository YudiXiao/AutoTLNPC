function [P_winding_Ldc,P_core_Ldc] = dc_inductor_loss(I_Lf_rms,...
                            V_Lf_PS_delta_t1,V_Lf_delta_t2,R_T,f_s,PS,...
                            delta_t1,delta_t2,core_k,core_a,core_b,N_T,...
                            Ae,Ve)
% The function dc_inductor_loss calculates the winding loss and the core 
% loss of the dc inductor.

    % Winding loss in W
    P_winding_Ldc = I_Lf_rms ^ 2 * R_T * N_T;
    ...
    v_signal(1,1:round((PS * T_s / 2 + delta_t1) / T_sample)) = V_Lf_PS_delta_t1;   
    v_signal(1,round((PS * T_s / 2 + delta_t1)/ T_sample) + 1:round((PS * T_s / 2 + ...
                    delta_t1 + delta_t2) / T_sample)) = V_Lf_delta_t2;
...
%     ylabel('|P1(f)|')
    ...
    % core loss in W
    P_core_Ldc = 1000 * P_core_v_total * Ve;
